...
if !command {
    -v clang - format& > / dev / null;
}
then echo "Error: clang-format is not installed." exit 1 fi
...
then
    VALID_FILES = "$VALID_FILES $file" else echo "Warning: File '$file' not found, skipping" fi
        done

#Если файлов для проверки нет, выходим
    if[-z "$VALID_FILES"];
then echo "No valid files to check." exit 0 fi
...
#-- style = file : использовать наш.clang - format
    echo "Running clang-format check..." clang -
    format-- dry - run - Werror-- style = file $VALID_FILES
...
do {
    if clang {
        -format-- dry - run-- style = file "$file" 2 > &1 | grep - q "clang-format";
    }
}
then echo "  - $file" fi
    done echo "" echo "To fix them automatically, run:" echo "  clang-format -i --style=file $VALID_FILES" echo "" echo "Or to fix specific files, run:" echo "  find src test -type f \( -name \"*.cpp\" -o -name \"*.h\" -o -name \"*.hpp\" \) -exec clang-format -i --style=file {} \;" exit 1 fi